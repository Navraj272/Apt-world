...
export class SetWithdrawalLimitHandler extends BaseHandler {
  async run() {
    const { userId, dailyWithdrawal, weeklyWithdrawal, monthlyWithdrawal, reset } = this.args;
    ...
    if (reset) {
      await db.UserLimit.update({ value: '' }, {
        where: {
          userId, key: {
            [Op.in]: [
              USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.DAILY_WITHDRAWAL_LIMIT,
              USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.WEEKLY_WITHDRAWAL_LIMIT,
              USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.MONTHLY_WITHDRAWAL_LIMIT
            ]
          }
        },
        transaction
    ...
    if ((weeklyLimit && dailyWithdrawal > weeklyLimit) || (monthlyLimit && dailyWithdrawal > monthlyLimit)) {
      throw new AppError(Errors.INVALID_GAMBLING_LIMIT, "Daily limit exceeds weekly or monthly limits.");
    }
    dailyLimit = dailyWithdrawal;

    if ((dailyLimit && weeklyWithdrawal < dailyLimit) || (monthlyLimit && weeklyWithdrawal > monthlyLimit)) {
    ...
    weeklyLimit = weeklyWithdrawal;

    if ((dailyLimit && monthlyWithdrawal < dailyLimit) || (weeklyLimit && monthlyWithdrawal < weeklyLimit)) {
      throw new AppError(Errors.INVALID_GAMBLING_LIMIT, "Monthly limit must be greater than or equal to daily and weekly limits.");
    }
    monthlyLimit = monthlyWithdrawal;
    ...
    limitsToUpdate.push({
      userId,
      key: USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.WEEKLY_WITHDRAWAL_LIMIT,
      value: weeklyLimit
    });
    limitsToUpdate.push({
      userId,
      key: USER_RESPONSIBLE_GAMBLING_LIMIT_TYPES.MONTHLY_WITHDRAWAL_LIMIT,
      value: monthlyLimit
    });

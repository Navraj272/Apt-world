    ...
    CREATE MATERIALIZED VIEW user_transaction_summary AS
      WITH
        casino_activity_summary AS (
          SELECT
            ct.user_id,
            MAX(DATE(ct.created_at)) AS transaction_date,
        ...
        banking_activity_summary AS (
          SELECT
            bt.user_id,
            MAX(DATE(bt.created_at)) AS transaction_date,
            SUM(CASE WHEN bt.purpose = 'purchase' AND bt.status = 'successful' AND bt.payment_provider != 'Offline' AND (bt.more_details->>'amount') IS NOT NULL
              THEN (bt.more_details->>'amount')::decimal ELSE 0 END) AS sc_purchased_amount,

            SUM(CASE WHEN bt.purpose = 'purchase' THEN bt.sc ELSE 0 END) AS sc_coin_purchased,
            SUM(CASE WHEN bt.purpose = 'purchase' AND bt.payment_provider = 'Offline' THEN bt.sc ELSE 0 END) AS sc_purchased_offline,
            SUM(CASE WHEN bt.purpose = 'redeem' AND bt.payment_provider = 'Offline' THEN bt.sc ELSE 0 END) AS sc_redeemed_offline,
            COUNT(CASE WHEN bt.purpose = 'purchase' AND bt.status = 'successful' AND bt.payment_provider != 'Offline' THEN 1 END) AS sc_purchased_count,
            SUM(CASE WHEN bt.purpose = 'redeem' AND bt.status IN ('successful', 'approved') AND bt.payment_provider != 'Offline' THEN bt.sc ELSE 0 END) AS sc_redeemed_amount,
        ...
        SELECT
          ...
          COALESCE(casino_activity_summary.transaction_date,banking_activity_summary.transaction_date) AS transaction_date,
          ...
          COALESCE(casino_activity_summary.total_game_played, 0) AS total_game_played,
        ...
        FULL OUTER JOIN banking_activity_summary ON casino_activity_summary.user_id = banking_activity_summary.user_id;
  `);

    await queryInterface.sequelize.query(`CREATE UNIQUE INDEX CONCURRENTLY idx_unique_user_transaction_summary ON user_transaction_summary (user_id);`)
  },

  async down(queryInterface, Sequelize) {
    // Drop the materialized view
    await queryInterface.sequelize.query(`DROP MATERIALIZED VIEW IF EXISTS user_transaction_summary;`);
  },
};

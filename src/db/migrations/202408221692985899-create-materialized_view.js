...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    await queryInterface.sequelize.query(`
      CREATE MATERIALIZED VIEW daily_user_transaction_summary AS
WITH
  casino_activity_summary AS (
    ...
    GROUP BY DATE(ct.created_at), ct.user_id, u.username
  ),
  ...
  banking_activity_summary AS (
    ...
    GROUP BY DATE(bt.created_at), bt.user_id, u.username
  ),
...
SELECT
  ...
  COALESCE(casino_activity_summary.username, banking_activity_summary.username) AS user_name,
...
FULL OUTER JOIN user_signup_summary
  ON COALESCE(casino_activity_summary.transaction_date, banking_activity_summary.transaction_date)
    ...
    await queryInterface.sequelize.query(`CREATE UNIQUE INDEX CONCURRENTLY idx_unique_daily_user_transaction_summary 
ON daily_user_transaction_summary (transaction_date);
`)
  ...
  async down(queryInterface, Sequelize) {
    ...
    await queryInterface.sequelize.query(`DROP MATERIALIZED VIEW IF EXISTS daily_user_transaction_summary;`);
  },
};
